...
from scripts.modal_mwe import client_mwe
...
def test_build_modal_lookup_same_hour_window() -> None:
    lookup = client_mwe._build_modal_lookup(
        app_id="ap-123",
...
def test_resolve_modal_actual_cost_not_yet_available(tmp_path) -> None:
    _write_summary(
        tmp_path,
        {
            "app_id": "ap-123",
    ...
    result = client_mwe.resolve_modal_actual_cost(
        output_dir=tmp_path,
...
def test_resolve_modal_actual_cost_complete(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(client_mwe, "_workspace_billing_report", fake_report)
